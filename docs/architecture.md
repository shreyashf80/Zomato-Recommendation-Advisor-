...
Presentation Layer
React + FastAPI Architecture (Selected & Active)
- **Frontend**: A decoupled React JS application bootstrapped with Vite and styled using Zomato's "Sushi" design system (specifically using Tailwind CSS, 'Plus Jakarta Sans' typography, a compact top filter bar, and a responsive multi-column restaurant cards grid).
- **Backend API**: A FastAPI application that loads the dataset, serves metadata endpoints (locations, cuisines), handles CORS, and forwards queries to the recommendation orchestrator.
- **Communication**: REST API calls (`POST /api/v1/recommendations`, `GET /api/v1/metadata/locations`, etc.) with JSON request/response payloads.

UI Flow
[Top Filter Bar] → [Submit] → [Pulsing Loading Skeletons Grid] → [AI Summary Banner] → [Ranked Multi-Column Restaurant Card Grid]
Form fields map 1:1 to UserPreferences. Location and cuisine dropdowns load dynamically. Cuisines and rating selectors are arranged in a horizontal grid. Cards are rendered vertically (image on top) in a 2-3 column responsive layout.
...
Technology Options
Selected Stack for the Zomato AI Recommendation Advisor:

| Concern | Stack Option (Selected) | Rationale |
| :--- | :--- | :--- |
| **Runtime** | Python 3.9+ | High performance for data operations and orchestration logic |
| **Linter / Type Checker** | Pyrefly | Rust-based high-performance static check validations |
| **UI Framework** | React.js (Vite + Tailwind CSS) | Implements custom Zomato "Sushi" Design System, responsive center-focused UI |
| **API Framework** | FastAPI (uvicorn) | Fast, typed, asynchronous endpoints with built-in OpenAPI/CORS support |
| **Data Library** | `datasets`, `pandas` | For loading and preprocessing the Hugging Face Zomato dataset |
| **Storage** | Parquet (In-memory Repository) | Direct read performance for filtered candidate slices |
| **LLM Provider** | Groq (`llama-3.3-70b-versatile`) | Fast inference and structured JSON response formatting |
| **Configuration** | `pydantic-settings` | Schema-driven environment variable validation |
